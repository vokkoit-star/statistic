...
def check_password():
    ...
    def login_handler():
        ...
        if st.session_state.get("input_username") == "vokko" and st.session_state.get("input_password") == "vokko":
    ...
    with st.form("auth_form"):
        username = st.text_input("Логин", key="input_username")
        password = st.text_input("Пароль", type="password", key="input_password")
        submit = st.form_submit_button("Войти", on_click=login_handler)
...
def load_supabase_data():
    try:
        ...
        try:
            ...
            supabase_url = "https://qinzjkipzmeiekfvosvy.supabase.co"
            supabase_key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFpbnpqa2lwem1laWVrZnZvc3Z5Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc0ODQ5NDEzMywiZXhwIjoyMDY0MDcwMTMzfQ.Ec2BSarhFoy3RiqJsXrSfxuewMLaoR2ky4rlYwiIdco"
            supabase: Client = create_client(supabase_url, supabase_key)
            
            # Получение данных из таблицы с дополнительной диагностикой
            response = supabase.table("AlmatinkaGorkogo").select("*").execute()
        except Exception as e:
            st.warning(f"Не удалось подключиться к Supabase: {str(e)}. Используем локальные данные.")
            # Загрузка из локального файла
            return pd.read_csv("supabase_data_dump.csv")
        
        # Проверка наличия данных
        if not response.data:
            st.error("Таблица 'AlmatinkaGorkogo' пуста или данные не получены")
            return None
            
        # Преобразование в DataFrame
        df = pd.DataFrame(response.data)
        
        # Переименование колонки времени
        if 'Дата и время' not in df.columns:
            st.error("Колонка 'Дата и время' отсутствует в данных")
            st.write("Доступные колонки:", df.columns.tolist())
            return None
            
        df = df.rename(columns={'Дата и время': 'Часы'})
        
        # Преобразование формата времени
        if not pd.api.types.is_datetime64_any_dtype(df['Часы']):
            try:
                df['Часы'] = pd.to_datetime(df['Часы'])
            except Exception as e:
                st.error(f"Ошибка преобразования времени: {str(e)}")
                return None
                
        # Конвертируем значения зон в числа
        zone_columns = [col for col in df.columns if col.startswith('Зона')]
        for col in zone_columns:
            df[col] = pd.to_numeric(df[col], errors='coerce')
        
        # Группировка по часам (агрегация средним значением)
        df['hour_group'] = df['Часы'].dt.floor('H')
        df = df.groupby('hour_group')[zone_columns].sum().reset_index()
        
        # Форматирование времени для отображения
        df['Часы'] = df['hour_group'].dt.strftime('%d.%m.%Y %H:00')
        df = df.drop(columns=['hour_group'])
        ...
        st.error(f"Ошибка при загрузке данных из Supabase: {str(e)}")
        return None
...
st.success("Данные успешно загружены из Supabase")
    ...
    if is_area_view:
        for i, area in enumerate(selected_items):
            ...
            if not available_zones:
                datasets.append({
                    "name": area,
                    "data": pd.Series([0] * len(filtered_data)),
                    "color": COLOR_PALETTE[i % len(COLOR_PALETTE)]
                })
                continue
            area_data = filtered_data[available_zones].apply(pd.to_numeric, errors='coerce').sum(axis=1)
            datasets.append({
                "name": area,
                "data": area_data,
                "color": COLOR_PALETTE[i % len(COLOR_PALETTE)]
        ...
        for i, zone in enumerate(selected_items):
            if zone not in filtered_data.columns:
                ...
                continue
            zone_data = pd.to_numeric(filtered_data[zone], errors='coerce').fillna(0)
            datasets.append({
                "name": zone,
                "data": zone_data,
                "color": COLOR_PALETTE[i % len(COLOR_PALETTE)]
...
if display_mode == "Одна дата":
    ...
    # Detect missing data (hours where all selected zones have zero values)
    missing_hours = []
    for index, row in filtered_data.iterrows():
        if view_mode == "Группы зон (улицы)":
            ...
            for area in selected_items:
                zones = AREA_GROUPS[area]
                zone_values.extend([pd.to_numeric(row[zone], errors='coerce') for zone in zones if zone in row])
        else:
            zone_values = [pd.to_numeric(row[zone], errors='coerce') for zone in selected_items if zone in row]
        if all(val == 0 or pd.isna(val) for val in zone_values):
            missing_hours.append(row['time'])
    ...
    # Add wider red markers for missing data hours (centered)
    for hour in missing_hours:
        fig.add_vline(
            x=hour,
            line_width=10,  # Increased from 3 to 6
            line_color="red",
            opacity=0.3,
            layer="above"
        )
    ...
    if missing_hours:
        st.warning(f"Пропуски данных обнаружены в следующих часах: {', '.join(missing_hours)}")

    # Prepare x-axis ticks: show non-missing hours, hide missing ones
    tickvals = [f"{h:02d}:00" for h in range(24)]
    ticktext = [f"{h:02d}:00" if f"{h:02d}:00" not in missing_hours else "" for h in range(24)]

    # Add red annotations for missing hours
    annotations = []
    for hour in missing_hours:
        hour_idx = tickvals.index(hour)
        annotations.append(
            dict(
                x=hour,
                y=0.02,  # Подняли подписи выше, ближе к оси X
                xref="x",
                yref="paper",
                text=f"{hour}",
                showarrow=False,
    ...
    if view_mode == "Группы зон (улицы)":
        ...
        for area in selected_items:
            zones = AREA_GROUPS[area]
            area_data = filtered_data[zones].apply(pd.to_numeric, errors='coerce').sum(axis=1)
            total_value = area_data.sum()
            max_hourly_value = area_data.max()
            efficiency = (total_value / (max_hourly_value * 24)) * 100 if max_hourly_value != 0 else 0
    ...
    # Detect missing data for both dates
    missing_hours1 = []
    for index, row in filtered_data1.iterrows():
        if view_mode == "Группы зон (улицы)":
            ...
            zone_values = [pd.to_numeric(row[zone], errors='coerce') for zone in selected_items if zone in row]
        if all(val == 0 or pd.isna(val) for val in zone_values):
            missing_hours1.append(row['time'])
    ...
    for index, row in filtered_data2.iterrows():
        if view_mode == "Группы зон (улицы)":
            ...
            zone_values = [pd.to_numeric(row[zone], errors='coerce') for zone in selected_items if zone in row]
        if all(val == 0 or pd.isna(val) for val in zone_values):
            missing_hours2.append(row['time'])
    ...
    if missing_hours1:
        st.warning(f"Пропуски данных для {date1}: {', '.join(missing_hours1)}")
    if missing_hours2:
        st.warning(f"Пропуски данных для {date2}: {', '.join(missing_hours2)}")

    # Combine missing hours from both dates
    combined_missing_hours = list(set(missing_hours1 + missing_hours2))

    # Prepare x-axis ticks: show non-missing hours, hide missing ones
    tickvals = [f"{h:02d}:00" for h in range(24)]
    ticktext = [f"{h:02d}:00" if f"{h:02d}:00" not in combined_missing_hours else "" for h in range(24)]

    # Add red vertical lines for missing hours
    shapes = []
    annotations = []
    for hour in combined_missing_hours:
        try:
            shapes.append(
                dict(
                    type="line",
            ...
            st.error(f"Ошибка создания линии для часа {hour}: {str(e)}")
    ...
    if view_mode == "Группы зон (улицы)":
        ...
        for i, area in enumerate(selected_items):
            zones = AREA_GROUPS[area]
            area_data1 = filtered_data1[zones].apply(pd.to_numeric, errors='coerce').sum(axis=1)
            total_value1 = area_data1.sum()
            max_hourly_value1 = area_data1.max()
            efficiency1 = (total_value1 / (max_hourly_value1 * 24)) * 100 if max_hourly_value1 != 0 else 0
            area_data2 = filtered_data2[zones].apply(pd.to_numeric, errors='coerce').sum(axis=1)
            total_value2 = area_data2.sum()
            max_hourly_value2 = area_data2.max()
            efficiency2 = (total_value2 / (max_hourly_value2 * 24)) * 100 if max_hourly_value2 != 0 else 0
